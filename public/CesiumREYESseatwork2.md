...
**The change that happened was that the "sidebar" block moved on the page. Instead of being in a fixed and strict position, it overlapped the content container. Whenever I change the values to move it around, it's able to overlap other elements instead of adjusting so that all the elements are still in a sort of "grid" structure and are just beside, on top or on the bottom of each other.**
...
**When I scroll the page, the footer doesn't really move along with it. It stays stationary, and I think that's how it behaves differently from position relative. When you use position:fixed, it orients the element relative to a browser's viewport so that when you scroll or move the browser, it does not change its positioning. This is different from position:relative, which is a bit more "fluid" and changes elements relative to how you orient or position the screen and other elements.**
...
**position:absolute orients items relative to their nearest position item whilst fixed orients it in terms of the viewport. Basically, position:absolute removes an elements from the normal flow of the layout and it can do things like easily overlap other elements, since it's kind of on a "separate layer."**
